# Excel テキスト置換ツール (v4.6) ユーザーマニュアル
...
*   **レポート出力**: 検索・置換の結果をCSVファイルとして出力します。
*   **安全設計**: 置換モードでは、変更箇所が青色・太字で強調保存されます。
*   **長いパス対応**: Windowsの深い階層にあるファイルも問題なく処理できます。

## インストール方法
特別なインストールは不要です。配布された `excel_converter_v4.6.exe` を任意のフォルダに配置してください。

## 使い方

### 1. アプリの起動
`excel_converter_v4.6.exe` をダブルクリックして実行します。
...
「レポートをダウンロード」ボタンを押すと、詳細な結果（CSVファイル）をダウンロードできます。
...
### Q. 起動しない（一瞬で閉じる）
A. すでにポート8080を使用している別のアプリがある可能性があります。PCを再起動するか、コマンドプロンプトから `excel_converter_v4.6.exe -port 8081` のように別のポートを指定して起動してみてください。
