# Excel テキスト置換ツール (v4.7) ユーザーマニュアル
...
## インストール方法
特別なインストールは不要です。配布された `excel_converter_v4.7.exe` を任意のフォルダに配置してください。

## 使い方

### 1. アプリの起動
`excel_converter_v4.7.exe` をダブルクリックして実行します。
...
### Q. 起動しない（一瞬で閉じる）
A. すでにポート8080を使用している別のアプリがある可能性があります。PCを再起動するか、コマンドプロンプトから `excel_converter_v4.7.exe -port 8081` のように別のポートを指定して起動してみてください。
